                    ...
                    switch ( decrypt_payload.command ) {
                        ...
                        case 'search':
                            ...
                            try {
                                ...
                                const arrString = fs.readdirSync( save_location )

                                // remove ".LOCK" files this happens if the sms server is trying to send messge but it takes a while to send it.
                                const files = arrString.filter( ( arr: string ) => !/.LOCK$/g.test( arr ) )
                            ...
                            try {
                                ...
                                */
                                files.map( ( arr: string , ind: number ) => {
                                    // remove the message payload and also its ".LOCK" files.
                                    fs.rmSync( `${ save_location }/${ arr }` , { recursive: true , force: true } )
                                    fs.rmSync( `${ save_location }/${ arr }.LOCK` , { recursive: true , force: true } )
                                } )
