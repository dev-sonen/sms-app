...
interface Props {
    ...
    sent_del: boolean       // for priviledges.
    setMenu: Function       // enable and disabled "menu" state.
}

export default function ListSent ( props: Props ): JSX.Element {
        ...
        <td scope='row' className='p-2 h-10 w-10'>
            ...
            <div className='flex items-center justify-center h-full w-full'>
                <input
                    ...
                    disabled={ disabled || sent_del ? false : true }
                />
            </div>

        </td>
        <td scope='row' className='p-2 h-10 w-28'>
            <div className='flex items-center justify-start h-full text-gray-800 font-open-sans-light w-24 truncate'>
                { obj.send_to }
            </div>
        </td>
    ...
    >
        <td scope='row' colSpan={ 7 } className='bg-gray-50 p-3 sm:p-7'>
            ...
            <div className='grid gap-5'>
                ...
                <div className='grid gap-1'>
                    <h5 className='text-blue-600 font-open-sans-semibold capitalize'>recipients:</h5> 
                    <span className='font-open-sans-light'>{ obj.send_to }</span>
                </div>
                <div className='block'>
                    <div className='flex space-x-1'>
                        <h5 className='text-blue-600 font-open-sans-semibold capitalize'>sender:</h5>
                        <p className='font-open-sans-light'>{ obj.sender }</p>
                    </div>
                    <div className='flex space-x-1'>
                        <h5 className='text-blue-600 font-open-sans-semibold capitalize'>message type:</h5>
                        <p className='font-open-sans-light'>{ obj.message_type }</p>
                    </div>
                    <div className='flex space-x-1'>
                        <h5 className='text-blue-600 font-open-sans-semibold capitalize'>flash enabled:</h5>
                        <p className='font-open-sans-light'>{ obj.flash === 1 ? 'yes' : 'no' }</p>
                    </div>
                    <div className='flex space-x-1'>
                        <h5 className='text-blue-600 font-open-sans-semibold capitalize'>datetime:</h5>
                        <p className='font-open-sans-light'>{ obj.datetime }</p>
                    </div>
                </div>
            </div>
