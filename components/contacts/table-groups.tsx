...
import ActionGroupsAdd from './action-groups-add'
import ActionGroupsDelete from './action-groups-delete'
import ActionGroupSearch from './action-groups-search'
...
export default function TableGroups ( props: Props ): JSX.Element {

    const {
        query,
        priviledges: { priviledges: { groups_add , groups_del , groups_edit } }
    } = props
                ...
                <thead className='sticky top-0 w-full bg-gray-100 text-xs'>
                    ...
                    <tr className='border-gray-100 border-b'>
                        ...
                        <th scope='col' className='p-2 h-10 w-44'>
                            <div className='flex items-center justify-start h-full text-gray-800 font-open-sans-regular uppercase'>id</div>
                        </th>
                        <th scope='col' className='p-2 h-10 w-44'>
                        ...
                        <th scope='col' className='p-2 h-10 w-max hidden md:table-cell'>
                            <div className='flex items-center justify-start h-full text-gray-800 font-open-sans-regular uppercase'>location</div>
                        </th>
                        <th scope='col' className='p-2 h-10 w-10'>
                        </th>
                    </tr>
                                        ...
                                        <td scope='row' colSpan={ 5 } className='p-2'>
                                            {
                                                length === 0
                                                    ?   <span className='text-gray-900 font-open-sans-light text-xs'>
                                                            you haven&apos;t created a group yet, click the + icon to create a group.
                                                        </span>
        ...
        <div className='flex items-center justify-center sm:justify-between'>
            ...
            <div className='flex space-x-1 items-center'>
                ...
                <Link
                    ...
                    href={ `/dashboard/contacts/groups?token=${ query.token }&pg=${ 1 }` }
                ...
                <Link
                    ...
                    href={ `/dashboard/contacts/groups?token=${ query.token }&pg=${ page - 1 }` }
                    className={ `flex items-center justify-center text-gray-700 ${ page === 1 || chunk.length === 0 ? 'pointer-events-none opacity-75' : 'cursor-pointer' } outline-none h-7 w-7 p-1 rounded transition ease-in-out duration-300 hover:bg-gray-200` }
                >
                ...
                </Link>
                {
                    instance.map( ( arr: any , ind: number ) => (                    
                        <Link
                            ...
                            href={ `/dashboard/contacts/groups?token=${ query.token }&pg=${ ind + 1 }` }
                            /*
                ...
                <Link
                    ...
                    href={ `/dashboard/contacts/groups?token=${ query.token }&pg=${ length }` }
                    className={ `flex items-center justify-center text-gray-700 ${ page === length || chunk.length === 0 ? 'pointer-events-none opacity-75' : 'cursor-pointer' } outline-none h-7 w-7 p-1 rounded transition ease-in-out duration-300 hover:bg-gray-200` }
                >
