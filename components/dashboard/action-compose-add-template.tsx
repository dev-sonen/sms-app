...
export default function ActionComposeAddTemplate ( props: Props ): JSX.Element {
            ...
            */
            e.target.id === 'window'
                &&  (
                        ...
                        setTimeout( () => setClasses( { ... classes , template: { display: 'hidden' , opacity: 'opacity-0' , translate: 'translate-x-10' } } ) , 300 )
                    )
        } }
        id='window' //
        className={ `fixed z-50 left-0 top-0 right-0 bottom-0 justify-end bg-black/25 ${ classes.template.display }` }
    >
        <div className={ `transition ease-in-out duration-300 ${ classes.template.opacity } ${ classes.template.translate }` }>
