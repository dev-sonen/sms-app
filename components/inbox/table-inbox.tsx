...
import ActionInboxDelete from './action-inbox-delete'
import ListInbox from './list-inbox'
...
export default function TableInbox ( props: Props ): JSX.Element {

    const {
        query,
        priviledges: { priviledges: { inbox_read , inbox_del } }
    } = props
                            ...
                            <div className='flex items-center justify-center h-full w-full'>
                                {
                                    chunk.length === 0
                                        ...
                                        :   <input
                                                ...
                                                disabled={ disabled || length === 0 || inbox_del ? false : true }
                                            />
                                }
                            </div>

                        </th>
                        <th scope='col' className='p-2 h-10 w-24'>
                            <div className='flex items-center justify-start h-full text-gray-800 font-open-sans-regular uppercase'>from</div>
                        </th>
                        <th scope='col' className='p-2 h-10 w-44 hidden md:table-cell'>
                            <div className='flex items-center justify-start h-full text-gray-800 font-open-sans-regular uppercase'>received</div>
                        </th>
                        <th scope='col' className='p-2 h-10 w-max'>
                            ...
                            ?   <tbody className='relative -z-10 bg-white text-xs'>
                                    <tr className='border-gray-100 border-b'>
                                        ...
                                        <td scope='row' colSpan={ 5 } className='p-2'>
                                            {
                                                length === 0
                                                    ?   <div className='flex items-center space-x-1 text-gray-900 font-open-sans-light text-xs'>inbox is empty.</div>
        ...
        <div className='flex items-center justify-center sm:justify-between'>
            ...
            <div className='flex space-x-1 items-center'>
                ...
                <Link
                    ...
                    href={ `/dashboard/inbox?token=${ query.token }&pg=${ 1 }` }
                ...
                <Link
                    ...
                    href={ `/dashboard/inbox?token=${ query.token }&pg=${ page - 1 }` }
                    className={ `flex items-center justify-center text-gray-700 ${ page === 1 || chunk.length === 0 ? 'pointer-events-none opacity-75' : 'cursor-pointer' } outline-none h-7 w-7 p-1 rounded transition ease-in-out duration-300 hover:bg-gray-200` }
                >
                ...
                </Link>
                {
                    instance.map( ( arr: any , ind: number ) => (                    
                        <Link
                            ...
                            href={ `/dashboard/inbox?token=${ query.token }&pg=${ ind + 1 }` }
                            /*
                ...
                <Link
                    ...
                    href={ `/dashboard/inbox?token=${ query.token }&pg=${ length }` }
                    className={ `flex items-center justify-center text-gray-700 ${ page === length || chunk.length === 0 ? 'pointer-events-none opacity-75' : 'cursor-pointer' } outline-none h-7 w-7 p-1 rounded transition ease-in-out duration-300 hover:bg-gray-200` }
                >
